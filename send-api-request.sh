...
# Usage: ./send-api-request.sh [-m HTTP_METHOD] [-c CONTENT_TYPE] [-p PARAM]... [-t TOKEN] <SERVER_URL> <ENDPOINT> <QUERY> [ADDITIONAL_PARAMETERS]
...
if [[ $# -lt 3 ]]; then
    ...
    echo "Usage: $0 [-m HTTP_METHOD] [-c CONTENT_TYPE] [-p PARAM]... [-t TOKEN] <SERVER_URL> <ENDPOINT> <QUERY> [ADDITIONAL_PARAMETERS]"
    exit 1
fi
...
TMPFILE=$(mktemp)
HTTP_CODE=$(curl -X "$HTTP_METHOD" --header "Authorization: Bearer ${TOKEN}" "${CONTENT_TYPE_HEADER[@]}" "${BODY_PARAM_ARGS[@]}" -o "$TMPFILE" -w "%{http_code}" "$FINAL_URL")
...
RESPONSE=$(echo "$RAW_RESPONSE" | jq '.' 2>/dev/null)

# Display the response and save to file
if [[ $? -ne 0 ]]; then
    echo "Warning: Response is not valid JSON. Printing raw response:"
    echo "$RAW_RESPONSE"
    echo "$RAW_RESPONSE" > "$TARGET_DIR/last-response.json"
else
    echo "$RESPONSE"
